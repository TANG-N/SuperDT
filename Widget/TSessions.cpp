...
TSessions::TSessions(QWidget *parent) : QWidget(parent)
{
    initVal();
    initUI();
}

void TSessions::initVal()
{
...
void TSessions::initUI()
{
    ...
    m_pTextEditorMain = new TSession(m_pSplitterMain);
...
QSplitter *TSessions::creatSplitter(Qt::Orientation orientation, QWidget *parent)
{
...
void TSessions::creatTextEditor(QWidget *parent)
{
...
void TSessions::removeSplitter(QSplitter *rmSplitter)
{
...
void TSessions::removeLastTextEditor()
{
    TSession *tmpTextEditor;
...
void TSessions::removeLastSplitter()
{
    QSplitter *tmpSplitter;
...
void TSessions::slotVSplitScreen()
{
    // Qt::Vertical   垂直
    // Qt::Horizontal 水平
    qDebug("垂直分屏");
...
void TSessions::slotHSplitScreen()
{
    qDebug("水平分屏");
    if(nullptr == m_pNowFocusTextEditor)
        return ;
...
void TSessions::slotMergeScreen()
{
    /* 大于两个编辑器的分屏策略
     * QSplitter Grandgrandfather,*Grandpa,*uncle,*parent;
     * TSession *brother *Me *uncleEditor;
     *    -----------------------------------------------------
     *  |                  Grandgrandfather                    |
     *  |  -----------------------------   -----------------   |
     *  | |          Grandpa            | |    Grandpa      |  |
...
//    {
//        m_pBrotherTextEditor = qobject_cast<TSession *>(m_pUncleSplitter->widget(0));//叔叔分割器仅有一个兄弟编辑器
...
void TSessions::slotFocusChange(QWidget *old, QWidget *now)
{
    TSession *oldTextEditor = qobject_cast<TSession *>(old);
    TSession *nowTextEditor = qobject_cast<TSession *>(now);

    /*过滤事件  获取当前光标所在TSession*/
